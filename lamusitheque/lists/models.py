...
def save_list_handler(sender, instance, created, **kwargs):
    if created:
        ...
        instance.save()
        itemlist.save()
        action.send(itemlist.user, verb="a ajouté", action_object=instance.album, target=instance.item_list,
                    to_str=add_item_activity(instance))


def add_item_activity(instance):
    return _link(instance.item_list.user) + " a ajouté l'album " + _link(instance.album) + " à sa liste " + _link(
        instance.item_list, instance.item_list.title)
...
def comment_list_notification(user, list):
    res = "<a href='{}'>{}</a>".format(reverse('profile', args=[user.username]),
                                       user.username) + " a commenté votre liste " + "<a href='{}'>{}</a>".format(
        reverse('lists:display_list', args=[list.id]), list.title)
    return res


def comment_list_notification_for_users(user, list):
    res = "<a href='{}'>{}</a>".format(reverse('profile', args=[user.username]),
                                       user.username) + " a commenté la liste " + "<a href='{}'>{}</a>".format(
        reverse('lists:display_list', args=[list.id]), list.title) + " de " + "<a href='{}'>{}</a>".format(
        reverse('profile', args=[list.user.username]), list.user.username)
    return res
...
def comment_list_reply_notification(comment, parent):
    ...
    res = "<a href='{}'>{}</a>".format(reverse('profile', args=[comment.user_name]),
        ...
        parent.get_absolute_url()) + " sur la liste " + "<a href='{}'>{}</a>".format(itemlist.get_absolute_url(),
                                                                                     itemlist.title) + " de " + "<a href='{}'>{}</a>".format(
        reverse('profile', args=[itemlist.user.username]), itemlist.user.username)
    return res
...
def notify_comment_list(comment, request, **kwargs):
    if comment.content_type == ContentType.objects.get_for_model(ItemList):
        ...
        if comment.user != user:
            notify.send(sender=comment.user, recipient=user, verb="a commenté votre liste", target=itemlist,
                        to_str=comment_list_notification(comment.user, itemlist))
        if comment.parent_id != 0:
            parent = XtdComment.objects.get(pk=comment.parent_id)
            parent_user = parent.user
            if parent_user != comment.user and parent_user != user:
                notify.send(sender=comment.user, recipient=parent_user, verb="a répondu à votre commentaire",
                            target=comment.xtd_comment, to_str=comment_list_reply_notification(comment, parent))
        else:
            parent_user = None
            users_in_thread = User.objects.filter(
                comment_comments__content_type=ContentType.objects.get_for_model(ItemList),
                comment_comments__object_pk=pk).exclude(pk=comment.user.pk).exclude(pk=user.pk).distinct()
            notify.send(sender=comment.user, recipient=users_in_thread, verb="a commenté la liste", target=itemlist,
                        to_str=comment_list_notification_for_users(comment.user, itemlist))
