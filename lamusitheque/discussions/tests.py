...
class DiscussionModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.instance = User.objects.create(username='Tata',
                                            password='23456')  # used as a content object for discussions (could be any other model)
...
class DiscussionViewTest(GenericAPITest):
    ...
    def setUp(self):
        ...
        self.instance = Album.objects.create(mbid=str(uuid.uuid4()), title='Test', album_type='SI')
        # used as a content object for discussions (could be any other model)
        self.discussion = Discussion.objects.create(user=self.user, content_object=self.instance, title='Title',
                                                    content='Content')
