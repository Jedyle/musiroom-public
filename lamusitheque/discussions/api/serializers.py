...
class DiscussionSerializer(serializers.ModelSerializer, DiscussionMixin, VoteSerializerMixin):
    ...
    def validate(self, data):
        ...
        if data.get('content_type') is not None:
            print(data)
            target = data['content_type'].model_class().objects.filter(id=data['object_id'])    
            if not target.exists():
                raise serializers.ValidationError("The target object does not exist")
        return data


class DiscussionUpdateSerializer(serializers.ModelSerializer, DiscussionMixin, VoteSerializerMixin):
    ...
    class Meta:
        model = Discussion
        fields = '__all__'
        read_only_fields = ('user', 'vote_score', 'num_vote_up', "num_vote_down", "content_object", 'content_type', 'object_id', 'created', 'modified')
