...
PATTERNS_TO_REMOVE = ["(Live)", "(EP)", "(Single)", "(OST)"]
...
class ParseSearch:
    def __init__(self, query, protocol=PROTOCOL, search=SEARCH, url=MUSICBRAINZ_URL):
        self.url = (
            ...
            + url
            + search
            + "?type=release_group&method=advanced&limit=10"
            + "&query="
            + query
        )
...
def parse_album_data(element):
    rating = element.find_element_by_css_selector(
        ".elco-collection-rating.user"
    ).get_attribute("innerText")
    product_detail = element.find_element_by_class_name("elco-product-detail")
    album = product_detail.find_element_by_class_name("elco-anchor").get_attribute(
        "innerText"
    )
    try:
        release_year = (
            product_detail.find_element_by_class_name("elco-date")
            .get_attribute("innerText")
            .replace("(", "")
            .replace(")", "")
        )
    ...
    artists = [
        ...
        for el in product_detail.find_elements_by_class_name("elco-baseline-a")
    ]
    return {
...
def parse_all_data(
    ...
    errorfile,
    driver,
    types=[LP_ID, EP_ID, LIVE_ID, COMPILATION_ID, SINGLE_ID, OST_ID],
):
    res = {}
    for type_el in types:
        res[TYPES[type_el]] = parse_data_for_type(
            user, type_el, filename, errorfile, driver
        )
    return res, driver
...
def compute_file(username, config, temp_dir=""):
    ...
    export, driver = parse_all_data(
        username, filename, errorfile, driver=driver, types=types
    )
