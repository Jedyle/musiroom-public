...
def notify_not_exported(username, *args, **kwargs):
    ...
    notify.send(sender=user, recipient=user, verb="export trop tôt",
...
def export_from_sc(username, sc_username, config, erase_old):
    ...
    try:
        with transaction.atomic():
            ...
            notify.send(sender=user, recipient=user, verb="a exporté ses données", target=report,
        ...
        notify.send(sender=user, recipient=user, verb="une erreur est survenue",
