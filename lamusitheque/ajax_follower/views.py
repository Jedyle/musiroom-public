...
def follow_notification(user):
    return "<a href={}>{}</a> ".format(reverse('user_profile', args=[user.username]), user.username) + "vous suit"
    ...
    if request.method == 'POST':
        ...
        if follow == 'true':
            if not Follow.objects.follows(user, user_to_follow):
                ...
                notify.send(sender=user, recipient=user_to_follow, verb="vous suit",
            ...
            if Follow.objects.follows(user, user_to_follow):
                ...
                notifications_alike(user_to_follow.notifications, actor=user, verb="vous suit").delete()
        return JsonResponse({})
