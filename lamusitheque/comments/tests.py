...
class CommentViewsetSet(GenericAPITest):
    ...
    def test_create_comment_loggedin(self):
        ...
        self.check_func_logged(self.create, auth_key=self.user1.auth_token.key, data=data, status_code=201)
    ...
    def check_affect_comment_wrong_auth(self, func, data):
        ...
        comment_id = self.comment1.id
        # login with user 1
        self.check_func_logged(func, id=comment_id, auth_key=self.user1.auth_token.key,
    ...
    def check_affect_comment_right_auth(self, func, data, status=200):
        ...
        comment_id = self.comment1.id
        # login with user 1
        self.check_func_logged(func, id=comment_id, auth_key=self.user2.auth_token.key,
                               data=data, status_code=status)

    def test_update_auth(self):
        data = {
            "comment": "This is a modification !"
        }
        func = self.update
    ...
    def test_create_comment_on_unauthorized_object(self):
        ...
        res = self.create({
            "content_type": ContentType.objects.get_for_model(unauthorized_obj).model,
