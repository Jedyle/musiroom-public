...
class ParseSearch:
    def __init__(self, query, search_type, page, protocol = PROTOCOL, search = SEARCH, url = MUSICBRAINZ_URL):
        self.url = protocol + url + search + "?query=" + query + "&type=" + search_type + "&method=indexed" + "&page=" + str(page)
...
class ParseArtist:
    ...
    def load(self):
        req = requests.get(self.url + self.artist_id + "?" + "filter.name=" + str(self.name) + "&" + "page=" + str(self.page))
        if req.status_code == 200:
            page = req.content
            self.soup = BeautifulSoup(page, "html.parser")
        return req.status_code == 200
