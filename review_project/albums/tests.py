...
class AlbumViewTest(TestCase):
    ...
    def setUp(self):
        album = Album.objects.create(mbid = 'f5093c06-23e3-404f-aeaa-40f72885ee3a', title = 'The Dark Side of the Moon') # DSOTM
    ...
    def test_404_if_not_album(self):
        response = self.client.get(reverse('albums:album', args=['c14b4180-dc87-481e-b17a-64e4150f90f6'])) #id for an artist
        self.assertEqual(response.status_code, 404)

    def test_404_if_not_exists(self):
        response = self.client.get(reverse('albums:album', args=['c14b4180-dc87-481e-b17a-64e4150f90f7'])) #does not exist
        self.assertEqual(response.status_code, 404)
    ...
    def test_create_if_not_exists(self):
        response = self.client.get(reverse('albums:album', args=['4e98c9b4-92f6-3049-b9da-a1088b623672'])) #Meddle by Pink Floyd
    ...
    def test_create_only_once(self):
        ...
        self.assertTrue(len(Album.objects.filter(mbid = 'f5093c06-23e3-404f-aeaa-40f72885ee3a')) == 1 and response.status_code == 200)
    ...
    def test_create_artist_if_not_exists(self):
        ...
        self.assertTrue(Artist.objects.filter(mbid='b10bbbfc-cf9e-42e0-be17-e2c3e1d2600d').exists()) #check if the artist was created
...
class ArtistViewTest(TestCase):
    ...
    def test_404_if_not_artist(self):
        response = self.client.get(reverse('albums:artist', args=['4ac65c55-08ac-3855-809e-07e7f4579a68'])) #id for an artist
        self.assertEqual(response.status_code, 404)

    def test_404_if_not_exists(self):
        response = self.client.get(reverse('albums:artist', args=['4ac65c55-08ac-3855-809e-07e7f4579a69'])) #does not exist
        self.assertEqual(response.status_code, 404)

    def test_200_if_exists(self):
        response = self.client.get(reverse('albums:artist', args=['8ca01f46-53ac-4af2-8516-55a909c0905e'])) #real artist
        self.assertEqual(response.status_code, 200)
