...
def notify_badge_awarded(badge_award, **kwargs):
    ...
    action.send(badge_award.user, verb='a recu le badge', action_object = badge_award, to_str="<a href='{}'>{}</a>".format(badge_award.user.get_absolute_url(), str(badge_award.user)) + ' a recu le badge ' + badge_award.name)
