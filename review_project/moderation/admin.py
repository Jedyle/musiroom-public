...
set_objects_as_pending.short_description = _("Changer le statut des objets sélectionnés à 'en attente'")
    ...
    def get_moderation_message(self, status=None, reason=None,
        ...
        if status == MODERATION_STATUS_PENDING:
            if visible_until_rejected:
                return _("L'objet est visible sur le site, "
                         "il sera supprimé si un modérateur le rejette")
            else:
                return _("L'objet n'est pas visible sur le site, "
                         "il sera visible si un modérateur l'accepte")
        elif status == MODERATION_STATUS_REJECTED:
            return _("Objet rejeté par le modérateur, "
                     "raison: %s" % reason)
        elif status == MODERATION_STATUS_APPROVED:
            return _("Objet accepté par le modérateur, "
                     "et visible sur le site")
        elif status is None:
            return _("Cet objet n'est pas enregistré sur "
